...
async function main() {
  ...
  console.log('Created users: admin, skding');
  ...
  const maintenanceTemplate = await prisma.checklistTemplate.create({
    data: {
  ...
  const customerTNB = await prisma.customer.create({
    data: {
  ...
  const customerKawan = await prisma.customer.create({
    data: {
  ...
  console.log('Created Customers: TNB Engineering, Kawan Engineering');

  // 5. Sample Initial Reports (Matching user sample docx)
